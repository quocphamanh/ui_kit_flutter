...
import 'package:izi_flutter_ui_kit/integrations/utils/constants.dart';
...
TextStyle boldFonts({color = blackColor, size = 16.0}) {
  return GoogleFonts.poppins(
      ...
      fontSize: size is int ? double.parse(size.toString()).toDouble() : size,
...
Widget getAppBar(context, String title,
    {color = primaryColor, textColor: whiteColor, List<Widget> actions}) {
  return AppBar(
    actions: actions,
    leading: IconButton(
...
Future<List<dynamic>> loadDataFromJson(BuildContext context) async {
  final fakeData =
      await DefaultAssetBundle.of(context).loadString('assets/map_point.json');
